...
export const ProfileView = ({
  ...
  const getBMICategory = (bmi: number) => {
    ...
    const pos = 75 + ((bmi - 30) / 10) * 25;
    return { label: 'Obesidad', color: 'bg-red-400', text: 'text-red-600', position: Math.min(pos, 100) };
  };
  ...
  const handlePredict = async () => {
    ...
    if (!formData.goal) {
      return alert('Por favor selecciona tu objetivo de salud.');
    }
    setIsLoading(true);
    try {
      ...
      if (res.status === 'success') {
        setTips(
          res.recommendations?.map((t: string, i: number) => ({
            id: i,
            text: t,
            completion: 0,   // completion % en lugar de status binario
          })) ?? []
        );
        setView('Dashboard');
        await refreshData();
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-8 animate-in fade-in duration-500">
      ...
      {/* Columna derecha: Preview + BMI */}
      <div className="lg:col-span-4 space-y-6">
        <div className="bg-[#4A5D44] p-6 rounded-[2rem] text-white shadow-xl">
          <p className="text-[9px] font-black uppercase tracking-widest opacity-60 mb-4 italic">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-[10px] opacity-60 uppercase">
                {formData.Age || '0'} años — {formData.Weight || '0'} kg
              </p>
              {formData.goal && (
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
              ...
              <p className="text-lg font-black">
                {currentBMI > 0 ? `${(formData.Height * 100).toFixed(0)} cm` : '0 cm'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-7 rounded-[2.5rem] border border-gray-100 shadow-sm text-center">
          ...
          <div className="flex justify-center items-baseline gap-1.5 mb-5">
            <span className="text-5xl font-black tracking-tighter text-gray-950">
          ...
          <div className={`p-4 rounded-xl ${bmiInfo.text} bg-gray-50 border border-gray-100 flex flex-col gap-1 items-start`}>
            <p className="text-[10px] font-black uppercase tracking-widest">
              Clasificación: {bmiInfo.label}
            </p>
            <p className="text-[9px] opacity-70 font-bold italic text-left">
              BMI {currentBMI > 0 ? currentBMI : '--'} — Referencia escala WHO 2020
            </p>
          </div>
        </div>
        ...
        <div className="bg-green-50 border border-green-200 p-4 rounded-2xl flex items-start gap-3 text-green-800">
          ...
          <p className="text-[10px] leading-relaxed font-semibold">
            <span className="font-bold">Aviso médico:</span> Este sistema no diagnostica enfermedades.
            Sus recomendaciones son orientativas y complementarias al seguimiento médico profesional.
          </p>
        </div>
      </div>
    </div>
